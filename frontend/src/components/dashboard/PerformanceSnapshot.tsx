...
import { performanceFixture, type PerformanceStat } from "@/data/dashboardFixtures";

function StatTile({ stat }: { stat: PerformanceStat }) {
  return (
    <div className="flex h-full flex-col gap-2 rounded-lg border border-line bg-inset px-4 py-4">
 ...
 * one-value chart is just a number wearing a costume.
 *
 * Closing line value is gated, and gated in `overlay` mode specifically — the
 * tile keeps its shape so a free user can see the measure exists and where it
 * would sit, rather than finding a gap in the row.
 */
    ...
    >
      ...
      <dl className="grid grid-cols-2 gap-2.5 sm:grid-cols-3 xl:grid-cols-2 2xl:grid-cols-3">
        {performanceFixture.map((stat) =>
          stat.feature ? (
            <PremiumGate key={stat.label} feature={stat.feature} mode="overlay">
              <StatTile stat={stat} />
            </PremiumGate>
          ) : (
